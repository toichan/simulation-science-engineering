...
double tmax = 0.5;  // シミュレーションの終わる時刻
...
int main ()
{
  ...
  for (istep=1; istep<=nstep; istep++){
    ...
    // 以下で unew をアップデートする。
    for (ix=1; ix<nx; ix++) { // まずは境界以外の点をループせよ ***************
      // FTCS 法に基づき unew[ix] を lambda, uold を用いて表せ **************
      unew[ix] = uold[ix] + lambda * (uold[ix+1] - 2.0*uold[ix] + uold[ix-1]);
    }
